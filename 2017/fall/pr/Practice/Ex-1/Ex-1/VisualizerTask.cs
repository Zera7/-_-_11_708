...
namespace Manipulation
{
	public static class VisualizerTask
	{
		...
		public static void KeyDown(Form form, KeyEventArgs key)
		{
            ...
            if (key.KeyCode == Keys.Q) Shoulder += Math.PI / 18;
            if (key.KeyCode == Keys.A) Shoulder -= Math.PI / 18;

            if (key.KeyCode == Keys.W) Elbow += Math.PI / 18;
            if (key.KeyCode == Keys.S) Elbow -= Math.PI / 18;
        ...
        public static void MouseMove(Form form, MouseEventArgs e)
		{
            ...
            PointF location = ConvertWindowToMath(e.Location, GetShoulderPos(form));
		...
		public static void DrawManipulator(Graphics graphics, PointF shoulderPos)
		{
            ...
            // Не забудьте сконвертировать координаты из логических в 
            PointF[] newLocation = new PointF[joints.Length];
            for (int i = 0; i < joints.Length; i++)
                newLocation[i] = ConvertMathToWindow(joints[i], shoulderPos);

            for (int i = 0; i < joints.Length - 1; i++)
            {
                graphics.DrawLine(ManipulatorPen, newLocation[i], newLocation[i + 1]);
                graphics.FillEllipse(JointBrush, newLocation[i].X - Radius, newLocation[i].Y - Radius, Radius * 2, Radius * 2);
            }
            graphics.DrawLine(ManipulatorPen, shoulderPos, newLocation[0]);
            graphics.FillEllipse(JointBrush, shoulderPos.X - Radius, shoulderPos.Y - Radius, Radius * 2, Radius * 2);
        }
		...
		private static void DrawReachableZone(Graphics graphics, Brush reachableBrush, Brush unreachableBrush, PointF shoulderPos, PointF[] joints)
		{
			...
			var mathCenter = new PointF(shoulderPos.X, shoulderPos.Y);
			var windowCenter = shoulderPos;
