...
namespace DiskTree
{
    public class DiskTreeTask
    {
        public class Root
        {
            ...
            {
                return Nodes.TryGetValue(subRoot, out Root node) ? node : Nodes[subRoot] = new Root(subRoot);
            }
            ...
            public List<string> MakeConcluson(int i, List<string> list)
            {
                ...
                foreach (var child in Nodes.Values.OrderBy(root => root.Name, StringComparer.Ordinal))
